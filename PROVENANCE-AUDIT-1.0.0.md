...
**Methodology revision:** 13 September 2026  
...
The September methodology revision does not change the audited release or the underlying package evidence. It clarifies how semantic identity is evaluated: Vintage Story namespaces, resource identifiers, dependency conditions and patch-operation types are treated as technically meaningful data and are not normalised away.

## Conclusion

The published mod is a consolidation and extension of the same compatibility area covered by the two earlier mods, so substantial overlap in purpose, authoritative entity paths, carcass mappings and required JSON structure is expected. The audit confirms that both earlier mods were used as acknowledged technical references.
...
- No shipped JSON file is semantically identical to either earlier mod when actual Vintage Story identifiers and patch semantics are preserved.
- Compatibility namespaces are not normalised for the purpose of determining semantic identity. A namespace is part of the resource identity and determines which mod-owned asset is being addressed or created.
- `add` and `addmerge` are treated as distinct Vintage Story patch operations. They are not collapsed into one another for semantic comparison.
- `dependsOn` conditions are retained because they determine when a patch is eligible to apply.
...
The primary comparison is semantics-preserving.

Formatting-only differences may be ignored when determining whether two JSON documents express the same data, but the comparison does **not** rewrite or discard technically meaningful Vintage Story data merely to increase apparent similarity.

In particular:

- compatibility namespaces and resource domains are preserved exactly;
- target asset paths and JSON patch paths are preserved exactly;
- `add`, `addmerge` and `replace` remain distinct operations;
- `dependsOn` conditions remain part of the operation being compared;
- referenced item, block, entity, texture and shape identifiers remain exact;
- values which control workload, mass, blood, storage, drops, quantities, shapes, textures or other behaviour remain exact.

This distinction matters because a namespace is not cosmetic text and `add`/`addmerge` are not stylistic alternatives. Normalising them can be useful as a deliberately permissive structural-similarity experiment, but it cannot establish technical or semantic identity.

The original audit also performed progressively more permissive stress tests which replaced compatibility namespaces, removed `dependsOn`, and finally treated `addmerge` as if it were `add`. Those tests are no longer used as identity tests in this provenance document because each removes information that has technical meaning in Vintage Story. Their former results do not alter the primary finding that the published files are not semantically identical under the game's actual identifiers and patch semantics.
...
| Semantically identical JSON files under actual Vintage Story identifiers and patch semantics | 0 | 0 | 0 |

The comparison above is the authoritative identity test used by this document.

Six simple mapping areas — Chelonioidea, Iniidae, Dasyuridae, Diprotodontidae, Thylacoleonidae and Vombatidae — have particularly close functional overlap with Tyeia's earlier compatibility coverage. That resemblance is expected because they target the same authoritative FotSA entity files and are intended to resolve to the same appropriate Butchering carcass outputs.

They are not classified as semantically identical merely because a deliberately altered comparison can make their structure converge. Namespace identity, dependency gating and Vintage Story patch-operation semantics remain part of the real data.
...
For operation-level identity, an operation is only treated as technically equivalent when its meaningful inputs remain equivalent, including the operation type, exact target resource, exact JSON path, dependency conditions and value being applied.

Accordingly:

- compatibility namespaces are not replaced with placeholders for the identity test;
- `addmerge` is not treated as `add`;
- `dependsOn` is not discarded;
- differing resource identifiers or values remain differences.

The previously quoted operation counts produced after namespace normalisation, and the additional counts produced by treating `addmerge` as `add`, were deliberately permissive similarity measurements. They are not retained here as technical-equivalence counts because the transformations used to obtain them remove real Vintage Story semantics.

The operation-level evidence therefore supports the same narrower conclusion as the whole-package comparison: there is substantial overlap in compatibility purpose, but that functional overlap must not be confused with semantic identity or line-for-line copying.

## Why the custom carcass files have the same shape

All ten subject areas which overlap Tyeia's compatibility coverage, together with the two carcass definitions retained from the compatibility area previously covered by Pepe, use the standard Butchering carcass schema. When the animal-specific item code is replaced with a placeholder, every one of the release's 12 carcass files has the same field layout as 12 native Butchering carcass definitions.
...
The namespace difference is technically real because it identifies a different compatibility package, but it is not presented here as an original gameplay concept or creative data choice.

The paired entity patch also changes `add` to `addmerge` and adds baby coverage. The `add`/`addmerge` difference is retained as a semantic difference because those are distinct Vintage Story patch operations, not interchangeable formatting choices.
...
6. Keep future provenance comparisons semantics-preserving: do not normalise namespaces, remove dependency gates, or collapse `add`, `addmerge` and `replace` when reporting technical identity.
7. Do not reorder or cosmetically rewrite constrained JSON merely to make it look different. Verifiable provenance and meaningful functional decisions are stronger evidence than superficial churn.

## Final finding

A fair description is:

> The project used both earlier compatibility mods as acknowledged technical references and necessarily retains much of the same functional mapping territory. Its implementation is evaluated using the actual Vintage Story namespaces, dependency conditions, resource identifiers and patch-operation semantics rather than by normalising those differences away. It removes obsolete duplicated content, redirects substantial coverage to current native Butchering definitions, changes data values throughout the retained custom carcasses, adds missing life stages and variants, fixes upstream compatibility defects, and introduces six new patch areas. The published package is not byte-identical, semantically identical under actual Vintage Story semantics, line-for-line identical or 99% identical to either earlier mod.
